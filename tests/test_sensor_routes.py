...
class SensorRoutesTestCases(unittest.TestCase):
    def setUp(self):
        ...
        cur.execute(
            "insert into readings (device_uuid,type,value,date_created) VALUES (?,?,?,?)",
            (self.device_uuid, "temperature", 100, int(time.time()) - 40),
        )
        cur.execute(
            "insert into readings (device_uuid,type,value,date_created) VALUES (?,?,?,?)",
    ...
    def test_device_readings_post(self):
        ...
        cur = conn.cursor()
        cur.execute(
            'select * from readings where device_uuid="{}"'.format(self.device_uuid)
        )
        rows = cur.fetchall()
    ...
    def test_device_readings_get_temperature(self):
        ...
        request = self.client().get(
            "/devices/{}/readings/?type=temperature".format(self.device_uuid)
        )
    ...
    def test_device_readings_get_humidity(self):
        ...
        """
        request = self.client().get(
            "/devices/{}/readings/?type=humidity".format(self.device_uuid)
        )
    ...
    def test_device_readings_get_past_dates(self):
        ...
        """
        request = self.client().get(
            "/devices/{}/readings/?type=temperature&start={}&end={}".format(
                self.device_uuid, int(time.time() - 100), int(time.time() - 40)
            )
        )
    ...
    def test_device_readings_min(self):
        ...
        """
        # Given a device UUID
        # When we make a request with the given UUID with query parameter of temperature
        request = self.client().get(
            "/devices/{}/readings/min/?type=temperature".format(self.device_uuid)
        )
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have  minimum sensor reading
        self.assertTrue(json.loads(request.data)[0]["value"], 22)
    ...
    def test_device_readings_max(self):
        ...
        # When we make a request with the given UUID with query parameter of temperature
        request = self.client().get(
            "/devices/{}/readings/max/?type=temperature".format(self.device_uuid)
        )
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have  maximum sensor reading
        self.assertTrue(json.loads(request.data)[0]["value"], 100)
    ...
    def test_device_readings_median(self):
        ...
        """
        request = self.client().get(
            "/devices/{}/readings/median/?type=temperature".format(self.device_uuid)
        )
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have median sensor reading
        self.assertTrue(json.loads(request.data)[0]["value"], 99.5)
    ...
    def test_device_readings_mean(self):
        ...
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have mean sensor reading
        self.assertTrue(json.loads(request.data)[0]["value"], 57.3)
    ...
    def test_device_readings_mode(self):
        ...
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have mode sensor reading
        self.assertTrue(json.loads(request.data)[0]["value"], 100)
    ...
    def test_device_readings_quartiles(self):
        ...
        """
        request = self.client().get(
            "/devices/{}/readings/quartiles/?type=temperature".format(self.device_uuid)
        )
        # Then we should receive a 200
        self.assertEqual(request.status_code, 200)
        # And the response data should have quartiles results for 1 and 3
        self.assertTrue(json.loads(request.data)["quartile_1"], 100)
